...
import type { InventoryItem, Supplier } from '../services/api';
...
const CreateSupplierOrder: React.FC = () => {
  ...
  const [suppliersList, setSuppliersList] = useState<SupplierDisplay[]>([]);
  const [selectedSupplier, setSelectedSupplier] = useState<SupplierDisplay | null>(null);
  const [quantity, setQuantity] = useState<string>('');
  const [expectedDelivery, setExpectedDelivery] = useState('');
  const [product, setProduct] = useState<InventoryItem | null>(null);
  const [showSupplierDetails, setShowSupplierDetails] = useState(false);
  const [supplierForDetails, setSupplierForDetails] = useState<SupplierDisplay | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Suppliers
        const approvedSuppliers = await inventoryService.getApprovedSuppliers();
        const mappedSuppliers = approvedSuppliers.map((s: Supplier) => ({
          id: s.id,
          name: s.company_name || s.name,
          address: s.address,
          email: s.email,
          phone: s.phone,
          categories: s.categories || 'N/A',
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        pricePerUnit: product.price * parseInt(quantity) // Using approximate logic as per existing usage
      };
      ...
      // Navigate back to updated location
      setTimeout(() => {
        navigate('/inventory');
      }, 2000);
    } catch (error: unknown) {
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500">Product not found or no product selected. Please go back to inventory.</p>
          <button
              ...
              <div
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer transition-colors ${
                  selectedSupplier?.id === supplier.id
                    ? 'border-l-4 border-wood-brown'
                    : 'hover:bg-gray-50'
                }`}
                style={selectedSupplier?.id === supplier.id ? { backgroundColor: 'rgba(139, 69, 19, 0.1)' } : undefined}
